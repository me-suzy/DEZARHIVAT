...
Description:
csDownload is an automated script that allows the user to manage file
downloads using custom forms. Build your own download 'mailto' forms without
having to learn Perl.

* Manage all your web site CGI file download 'mailto' forms from one location.
* Easy to use management screen.
* Choose directories on your server that contain the files to download.
* Select from the files within the chosen download directory and add file
  descriptions.
* Choose from a pre-determined list of fields or create any number of your own
  fields:
  Text Area
  Drop Down
  Text Box
...
1. Unzip the file and transfer to your server with the following
   ...
   /cgi-script/CSDownload <-- CSDownload directory
   /cgi-script/CSDownload/export <-- export file directory.
   /cgi-script/CSDownload/forms <-- forms directory.

  CSDownload.cgi should be located at 
  [root url]/cgi-script/CSDownload/CSDownload.cgi where [root url]
  is the URL to your website. Example:
  http://www.mycompany.com/cgi-script/CSDownload/CSDownload.cgi

2. Chmod CSDownload.cgi to executable (chmod 755)
   Ask your server administrator for assistance if required.

3. If your perl executable is not located at /usr/bin/perl, edit
   the first line of CSDownload.cgi to reflect the true location.

4. Edit the following variables in the file setup.cgi (descriptions below):
     $cgipath = '/www/vhosts/cgiscript.net/cgi-script/CSDownload/';
     $cgiurl = 'http://www.cgiscript.net/cgi-script/CSDownload/';
     $defaultdir = '/www/vhosts/cgiscript.net/';
     $sendmail = '/usr/sbin/sendmail';
     $username = 'demo';
     $password = 'demo';

$cgipath is the path the the CSDownload DIRECTORY.

$cgiurl is the URL to the CSDownload DIRECTORY.

$defaultdir is the path to the default directory that you are taken
to when you browse for a directory containing your downloads. In most cases,
this will be the path to the root directory of your site.
...
Creating A New Form.
1. Go to [$cgiurl]/CSDownload.cgi where [$cgiurl] is the URL to the CSDownload
...
3. Identify the directory where all your download files are located. Click the
   'Directory Path' button to browse for the directory or manually type in
   the full path. If you type in the full path, click the 'refresh' button to
   view the files within that directory.

4. To have a file added for download, enter a description for the file. If no
   description is given for the file then it will not be included on the form.

5. Click the 'Create Form' button at the bottom of the page when finished. 
