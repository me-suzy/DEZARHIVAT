...
Filename: include/javascript.php
...
function over_button ($link, $image1, $image2, $area_id="", $target="", $alt="")
{
	...
	if ($area_id)
	{
		if ($area_id != $id) 
		{
			return 	"<a href=\"$link\"$target onMouseOut=\"swapImgRestore()\" ".
			"onMouseOver=\"swapImage('$name','','$image2',1)\">".
			"<img name=$name border=0 src=\"$image1\" alt=\"$alt\">".
			"</a>";
		} 
		else
		{
			return 	"<a href=\"$link\"$target onMouseOut=\"swapImgRestore()\" ".
			"<img name=$name border=0 src=\"$image2\" alt=\"$alt\">".
			"</a>";
		}
