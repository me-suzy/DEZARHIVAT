...
// prints a form with the account fields
// then we redirect the user to $url if everything is ok
// if $submit_image is set, it is used as a graphical
// submit button, or,
// if its not a image then it is used as the button string

function print_edit_account_form ($url="", $submit_image="")
{
	global $skin;
	global $id;
	
	
	// get fields that were required to register
	$query = mysql_query ("	SELECT field_name, field_type, code_register FROM user_fields
	WHERE required_to_register=1
	ORDER BY id");
	...
	if (mysql_num_rows($query))
	{
		...
		while (list ($field_name, $type, $code) = mysql_fetch_row($query))
		{
			$$field_name = get_user_data ($field_name);
			
			if ($type == "date")
			{
				$name_day = $field_name."_day";
				$name_month = $field_name."_month";
				$name_year = $field_name."_year";
				
				list ($$name_year, $$name_month, $$name_day) = explode ("-", $$field_name);
			}
...
// prints "selected" if $value equals $field_value
function selected ($field_value="", $value="")
{
	if ($field_value == $value) print " selected";
}
?>
